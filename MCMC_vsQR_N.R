...
## MCMC
vsQR_re_gaussian <- function(y,x,idobs,tau,n_mcmc,burnin_mcmc,thin_mcmc){
  n <- length(y)
  numcov <- ncol(x)
  ...
  for(k in 2:n_mcmc){
    ...
    PI.aux     <- atualizarPI(numcov,PI[k-1,],y,alpha_aux,x,beta[k-1,],sigma[k-1,1],delta[k,1],B)
    PI[k,]     <- PI.aux[1,]
    PROB[k,]   <- PI.aux[2,]
    n_select   <- which(PI[k,]==1)
    beta[k,n_select] <- atualizarBETA(b[n_select],B[n_select,n_select],alpha_aux,x[,n_select],sigma[k-1,1],y)
    ######################
    sigma[k,1] <- atualizarSIGMA(0.01,0.01,alpha_aux,x,beta[k,],y,n)
    alpha[k,]  <- atualizarALPHA(y,x,beta[k,],sigma[k,1],malpha[k-1,1],psi2[k-1,1],t(m_alpha),n_idobs)
...
atualizarBETA<-function(b,B,alpha,x,sigma,y){
  B.inv  <- chol2inv(chol(B))
  covar  <- chol2inv(chol(B.inv+(1/(sigma)*(t(x)%*%x))))
  media  <- covar%*%((B.inv%*%b)+(1/(sigma))*(t(x)%*%(y-alpha)))
  beta   <- rmvnorm(1,media,covar)
  return(beta)
}

# Full conditional for sigma
atualizarSIGMA<-function(c,C,alpha,x,beta,y,N){
  alpha1 <- c + 0.5*N
  beta1  <- C + 0.5*(t((y-alpha-x%*%beta))%*%(y-alpha-x%*%beta))
  sigma  <- 1/rgamma(1, alpha1, beta1)
  return(sigma)
}

# Full conditional for the random effects
atualizarALPHA<-function(y,x,beta,sigma,alpha_mean,psi2,m_aux,N){
  z     <- y-x%*%beta
...
atualizarPI<-function(numcov,PI,y,alpha,covariaveis,beta,sigma,delta,Bjj){
  chance <- NULL
